...
public class FileBoardController {
	...
	public void downloadFile(long boardIdx, long idx, HttpServletResponse response) throws Exception {
		BoardFileEntity file=fileBoardService.getFile(boardIdx,idx);
		//System.out.println(file);
		String path="/upload"+file.getStoredfilePath();
		File downFile=new File(path);
		byte[] fileBytes=org.apache.commons.io.FileUtils.readFileToByteArray(downFile);
		response.setContentType("application/octet-stream");//바이너리 파일
		response.setContentLength(fileBytes.length);
		response.setHeader("Content-Disposion", "attachment; fileName='"+ URLEncoder.encode(file.getOrignalFileName(),"UTF-8")+"';");
	...
	@GetMapping("fboard/delete/{boardIdx}")
	public String boardDelete(@PathVariable("boardIdx")long boardIdx) {
		fileBoardService.boardDelete(boardIdx);
		return "redirect:/fboard";
		
	}
	
	@PostMapping("/fboard/edit")
